...
// V32 (DSv3.2) decode v2 — forked from decode-dsv4. Warp-spec (1 IO + 8 math
...
#include <flashinfer/attention/sparse_mla_sm120/decode_dsv3_2_v2_kernel.cuh>
...
static bool launch_decode_dsv3_2_v2_impl(
  ...
  // Dynamic smem layout (must match decode_dsv3_2_v2_kernel.cuh exactly).
  ...
  constexpr int DYN_SMEM_BYTES =
      ...
      + V2_KV_BUF_COUNT * V2_BI * KV::KV_SMEM_STRIDE                    // sm_kv_fp8 ×2 (NoPE + INLINE scales)
      + V2_KV_BUF_COUNT * V2_BI * KV::D_ROPE * (int)sizeof(bf16)        // sm_kv_rope ×2
      + 16                                                              // mbar align pad
      + 4 * (int)sizeof(uint64_t)                                       // mbar_full + mbar_empty
      + 2 * V2_N_WARPS * HPB * (int)sizeof(float)                       // sm_reduce
      + N_V_CHUNKS_LAUNCH * HPB * (int)sizeof(float)                    // sm_w_head_sc
      + 2 * HPB * (V2_BI + 16);                                         // sm_w_fp8 ×2

  auto kernel = sparse_mla_decode_dsv3_2_v2_kernel<NUM_HEADS, TOPK, 64>;
  V2_CUDA_CHECK(cudaFuncSetAttribute(
  ...
  if (chunks_per_block_override >= 1 && chunks_per_block_override <= num_splits) {
    ...
    int sm_count = 0;
    V2_CUDA_CHECK(cudaDeviceGetAttribute(&sm_count, cudaDevAttrMultiProcessorCount, 0));
  ...
  V2_CUDA_CHECK(cudaGetLastError());
...
// Public surface: V32 (DSv3.2) decode v2.
...
#define V2_DISPATCH(H, K)                                                       \
  if (num_heads == (H) && topk == (K)) {                                        \
    return launch_decode_dsv3_2_v2_impl<(H), (K)>(                              \
  ...
  V2_DISPATCH(128, 128)
  V2_DISPATCH(128, 512)
  V2_DISPATCH(128, 1024)
  V2_DISPATCH(128, 2048)
#undef V2_DISPATCH
  return false;
}
